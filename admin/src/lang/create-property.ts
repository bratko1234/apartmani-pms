import LocalizedStrings from 'localized-strings'
import env from '@/config/env.config'
import * as langHelper from '@/utils/langHelper'

const strings = new LocalizedStrings({
  fr: {
    NEW_PROPERTY_HEADING: 'Nouvelle propriété',
    NAME: 'Nom',
    PROPERTY_IMAGE_SIZE_ERROR: `L'image doit être au format ${env.PROPERTY_IMAGE_WIDTH}x${env.PROPERTY_IMAGE_HEIGHT}`,
    RECOMMENDED_IMAGE_SIZE: `Taille d'image recommandée : ${env.PROPERTY_IMAGE_WIDTH}x${env.PROPERTY_IMAGE_HEIGHT}`,
    AGENCY: 'Agency',
    LOCATION: 'Locatlisation',
    AVAILABLE: 'Disponible à la location',
    PROPERTY_TYPE: 'Type',
    PRICE: 'Prix',
    MINIMUM_AGE: 'Âge minimum',
    MINIMUM_AGE_NOT_VALID: `L'âge minimum doit être supérieur ou égal à ${env.MINIMUM_AGE} ans.`,
    ADDRESS: 'Adresse',
    DESCRIPTION: 'Description',
    BEDROOMS: 'Chambres à couche',
    BATHROOMS: 'Salles de bain',
    KITCHENS: 'Cuisines',
    PARKING_SPACES: 'Parkings',
    SIZE: 'Superficie',
    AIRCON: 'Climatisation',
    FURNISHED: 'Meublée',
    PETS_ALLOWED: 'Animaux domestiques',
    SOLD_OUT: 'Épuisée',
    HIDDEN: 'Cachée',
    IMAGES: 'Images',
    DESCRIPTION_REQUIRED: 'Le champ description est requis',
    RENTAL_TERM: 'Durée de location',
    BLOCK_ON_PAY: 'Bloquer la propriété après un paiement réussi',
    IS_BUILDING: 'Bâtiment / Hôtel (contient des types de chambres)',
    PARENT_PROPERTY: 'Bâtiment parent',
    COUNT_OF_ROOMS: 'Nombre de chambres identiques',
    NO_PARENT: 'Aucun (propriété autonome)',
    ROOM_TYPES: 'Types de chambres',
    ADD_ROOM_TYPE: 'Ajouter un type de chambre',
    NO_ROOM_TYPES: 'Aucun type de chambre défini',
  },
  en: {
    NEW_PROPERTY_HEADING: 'New property',
    NAME: 'Name',
    PROPERTY_IMAGE_SIZE_ERROR: `The image must be in the format ${env.PROPERTY_IMAGE_WIDTH}x${env.PROPERTY_IMAGE_HEIGHT}`,
    RECOMMENDED_IMAGE_SIZE: `Recommended image size: ${env.PROPERTY_IMAGE_WIDTH}x${env.PROPERTY_IMAGE_HEIGHT}`,
    AGENCY: 'Agency',
    LOCATION: 'Location',
    AVAILABLE: 'Available for rental',
    PROPERTY_TYPE: 'Type',
    PRICE: 'Price',
    MINIMUM_AGE: 'Minimum age',
    MINIMUM_AGE_NOT_VALID: `Minimum age must be greater than or equal to ${env.MINIMUM_AGE} years old.`,
    ADDRESS: 'Address',
    DESCRIPTION: 'Description',
    BEDROOMS: 'Bedrooms',
    BATHROOMS: 'Bathrooms',
    KITCHENS: 'Kitchens',
    PARKING_SPACES: 'Parking spaces',
    SIZE: 'Size',
    AIRCON: 'Aircon',
    FURNISHED: 'Furnished',
    PETS_ALLOWED: 'Pets allowed',
    SOLD_OUT: 'Sold out',
    HIDDEN: 'Hidden',
    IMAGES: 'Images',
    DESCRIPTION_REQUIRED: 'Description is required',
    RENTAL_TERM: 'Rental term',
    BLOCK_ON_PAY: 'Block Property On Successful Payment',
    IS_BUILDING: 'Building / Hotel (contains room types)',
    PARENT_PROPERTY: 'Parent Building',
    COUNT_OF_ROOMS: 'Number of identical rooms',
    NO_PARENT: 'None (standalone property)',
    ROOM_TYPES: 'Room Types',
    ADD_ROOM_TYPE: 'Add Room Type',
    NO_ROOM_TYPES: 'No room types defined yet',
  },
  sr: {
    NEW_PROPERTY_HEADING: 'Nova nekretnina',
    NAME: 'Naziv',
    PROPERTY_IMAGE_SIZE_ERROR: `Slika mora biti u formatu ${env.PROPERTY_IMAGE_WIDTH}x${env.PROPERTY_IMAGE_HEIGHT}`,
    RECOMMENDED_IMAGE_SIZE: `Preporučena veličina slike: ${env.PROPERTY_IMAGE_WIDTH}x${env.PROPERTY_IMAGE_HEIGHT}`,
    AGENCY: 'Agencija',
    LOCATION: 'Lokacija',
    AVAILABLE: 'Dostupno za iznajmljivanje',
    PROPERTY_TYPE: 'Tip',
    PRICE: 'Cijena',
    MINIMUM_AGE: 'Minimalna starost',
    MINIMUM_AGE_NOT_VALID: `Minimalna starost mora biti veća ili jednaka ${env.MINIMUM_AGE} godina.`,
    ADDRESS: 'Adresa',
    DESCRIPTION: 'Opis',
    BEDROOMS: 'Spavaće sobe',
    BATHROOMS: 'Kupatila',
    KITCHENS: 'Kuhinje',
    PARKING_SPACES: 'Parking mjesta',
    SIZE: 'Veličina',
    AIRCON: 'Klima uređaj',
    FURNISHED: 'Namješteno',
    PETS_ALLOWED: 'Kućni ljubimci dozvoljeni',
    SOLD_OUT: 'Rasprodato',
    HIDDEN: 'Skriveno',
    IMAGES: 'Slike',
    DESCRIPTION_REQUIRED: 'Polje opisa je obavezno',
    RENTAL_TERM: 'Period iznajmljivanja',
    BLOCK_ON_PAY: 'Blokiraj nekretninu nakon uspješnog plaćanja',
    IS_BUILDING: 'Zgrada / Hotel (sadrži tipove soba)',
    PARENT_PROPERTY: 'Matična zgrada',
    COUNT_OF_ROOMS: 'Broj identičnih soba',
    NO_PARENT: 'Nijedno (samostalna nekretnina)',
    ROOM_TYPES: 'Tipovi soba',
    ADD_ROOM_TYPE: 'Dodaj tip sobe',
    NO_ROOM_TYPES: 'Još nema definisanih tipova soba',
  },
})

langHelper.setLanguage(strings)
export { strings }
